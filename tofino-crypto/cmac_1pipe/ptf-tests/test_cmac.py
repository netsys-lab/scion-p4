...
from controller.cmac import CmacPipe, CmacTestPipe
...
class Cmac2PipesTest(BaseTest):
    ...
    class Pipe0(BfRuntimeTest):
        def setUp(self, client_id, p4_name="cmac_1pipe"):
            BfRuntimeTest.setUp(self, client_id, p4_name)
            self.bfrt_info = self.interface.bfrt_info_get(p4_name)
            self.controller = CmacPipe(self.interface, p4_name)
    ...
    class Pipe1(BfRuntimeTest):
        def setUp(self, client_id, p4_name="cmac_1pipe_test"):
            BfRuntimeTest.setUp(self, client_id, p4_name)
            self.bfrt_info = self.interface.bfrt_info_get(p4_name)
            self.controller = CmacTestPipe(self.interface, p4_name)
    ...
    def runTest(self):
        ...
        verify_packet(self, payload, _make_port(1, 9))
        ...
        send_packet(self, _make_port(1, 8), pkt)
        verify_no_packet(self, payload, _make_port(1, 9))
