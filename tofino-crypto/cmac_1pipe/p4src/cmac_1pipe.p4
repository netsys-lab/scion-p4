    ...
    inout ingress_intrinsic_metadata_for_tm_t       ig_tm_md)
{
    ...
    ST_TABLES(0, 4, hdr.block0)
    ST_TABLES(1, 4, hdr.block1)
    ...
    apply {
        if (hdr.meta.isValid()) {
            // Round 4 and 10
            if (hdr.block0.isValid()) {
                APPLY_ST_TABLES(0, 4, hdr.block0);
                ADD_ROUND_KEY(0, 4);
            }
            if (hdr.block1.isValid()) {
                APPLY_ST_TABLES(1, 4, hdr.block1);
                ADD_ROUND_KEY(1, 4);
            }
            ...
            if (hdr.meta.iter == 0) {
                ...
                if (pass) {
                    ...
                    hdr.meta.setInvalid();
                    hdr.block0.setInvalid();
                    hdr.block1.setInvalid();
                }
            }
        }
