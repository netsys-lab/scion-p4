...
from controller.aes import AesFirstPipe, AesSecondPipe, AesTestPipe
...
class Aes2PipesTest(BaseTest):
    ...
    class Pipe0(BfRuntimeTest):
        def setUp(self, client_id, p4_name="aes_pipe0"):
            BfRuntimeTest.setUp(self, client_id, p4_name)
            self.bfrt_info = self.interface.bfrt_info_get(p4_name)
            self.controller = AesFirstPipe(self.interface)
    ...
    class Pipe1(BfRuntimeTest):
        def setUp(self, client_id, p4_name="aes_pipe1"):
            BfRuntimeTest.setUp(self, client_id, p4_name)
            self.bfrt_info = self.interface.bfrt_info_get(p4_name)
            self.controller = AesSecondPipe(self.interface)
    ...
    class Pipe2(BfRuntimeTest):
        def setUp(self, client_id, p4_name="aes_test"):
            BfRuntimeTest.setUp(self, client_id, p4_name)
            self.bfrt_info = self.interface.bfrt_info_get(p4_name)
    ...
    def runTest(self):
        ...
        verify_packet(self, pkt, _make_port(2, 8))
        send_packet(self, _make_port(2, 9), pkt)
        verify_packet(self, pkt, _make_port(2, 9))
        ...
        exp = ether / BridgeHeader(Flags=BrFlags.ENB_BLOCK0,
            Block0=0x3925841d02dc09fbdc118597196a0b32,
            Key=0xd014f9a8c9ee2589e13f0cc8b6630ca6
        )
        send_packet(self, _make_port(2, 8), pkt)
        verify_packet(self, exp, _make_port(2, 8))
        ...
        exp = ether / BridgeHeader(Flags=BrFlags.ENB_BLOCK0 | BrFlags.ENB_BLOCK1,
            Block0=0x3925841d02dc09fbdc118597196a0b32,
        ...
        for i, key in enumerate(keys):
            ...
            exp = Mask(ether / BridgeHeader(Flags=BrFlags.ENB_BLOCK0 | BrFlags.ENB_BLOCK1,
                Block0=block0[i + 1],
                Block1=block1[i + 1],
            ))
