    ...
    inout egress_intrinsic_metadata_for_output_port_t eg_oport_md)
{
    ...
    apply {
        if (hdr.key.isValid()) {
            ...
            // Calculate key for round 1 and 6
            ...
            // Calculate key for round 2 and 7
            ...
            // Calculate key for round 3 and 8
            ...
            // Calculate key for round 4 and 9
    ...
    inout ingress_intrinsic_metadata_for_tm_t       ig_tm_md)
{
    ...
    ST_TABLES(0, 5, hdr.block0)
    ST_TABLES(1, 5, hdr.block1)
    ...
    apply {
        if (hdr.key.isValid()) {
            ...
            // Calculate key for round 5 and 10
            EXPAND_KEY(0, 5, hdr.key);
            EXPAND_KEY(1, 5, meta.key);

            // Round 5 and 10
            if (hdr.block0.isValid()) {
                APPLY_ST_TABLES(0, 5, hdr.block0);
                ADD_ROUND_KEY(0, hdr.key);
            }
            if (hdr.block1.isValid()) {
                APPLY_ST_TABLES(1, 5, hdr.block1);
                ADD_ROUND_KEY(1, meta.key);
            }
        ...
        // Recirculate of forward to next pipe
        tab_forward.apply();
    }
}
