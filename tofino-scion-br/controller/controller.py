...
from cmac_2pipes.controller.cmac import CmacFirstPipe, CmacSecondPipe
from cmac_1pipe.controller.cmac import  CmacPipe

logger = logging.getLogger('scion')
...
def main():
    ...
    if args.aes_pipes:
        if args.aes_pipes[1] == args.aes_pipes[2]:
            tofino_if.bind_pipeline_config("cmac_1pipe")
            cmac = CmacPipe(interface=tofino_if, pipe=int(args.aes_pipes[1]))
            cmac.clear_tables()
            cmac.program_tables()
            cmac.set_key(key)
        else:
            tofino_if.bind_pipeline_config("cmac_pipe0")
            cmac1 = CmacFirstPipe(interface=tofino_if, pipe=int(args.aes_pipes[1]), next_pipe=int(args.aes_pipes[2]))
            cmac1.clear_tables()
            cmac1.program_tables()
            cmac1.set_key(key)

            tofino_if.bind_pipeline_config("cmac_pipe1")
            cmac2 = CmacSecondPipe(tofino_if, pipe=int(args.aes_pipes[2]))
            cmac2.clear_tables()
